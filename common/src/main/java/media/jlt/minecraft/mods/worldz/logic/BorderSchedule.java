 ...
 * @param style whether the rate fields drive one smooth lerp or abrupt jumps
 */
public record BorderSchedule(
    ...
    public BorderSchedule {
        ...
        if (style == ResizeStyle.STEPPED && resizeRateBlocks == 0) {
    ...
    public BorderSchedule(int initialRadiusBlocks, int finalRadiusBlocks, int resizeDays) {
        this(initialRadiusBlocks, finalRadiusBlocks, resizeDays, 0, 0, 0, ResizeStyle.CONTINUOUS);
    }
    ...
    public BorderSchedule(
        ...
        this(initialRadiusBlocks, finalRadiusBlocks, resizeDays, 0, resizeRateBlocks, resizeRateDays, ResizeStyle.CONTINUOUS);
    }

    /**
     * Creates a continuous-style schedule with an initial delay.
     *
     * @param initialRadiusBlocks center-to-side distance at creation
     * @param finalRadiusBlocks center-to-side distance at completion
     * @param resizeDays legacy total transition duration
    ...
    public BorderSchedule(
        ...
        this(initialRadiusBlocks, finalRadiusBlocks, resizeDays, resizeDelayDays, resizeRateBlocks, resizeRateDays, ResizeStyle.CONTINUOUS);
    }
     ...
     * A zero-day schedule immediately uses the final radius. {@code STEPPED} schedules
     * jump by {@link #resizeRateBlocks} every {@link #resizeRateDays} rather than
    ...
    public double radiusAtTick(long elapsedTicks) {
        ...
        if (style == ResizeStyle.STEPPED) {
            return steppedRadiusAtTick(transitionTicks);
        }
    ...
    private double steppedRadiusAtTick(long transitionTicks) {
        long distance = Math.abs((long)finalRadiusBlocks - initialRadiusBlocks);
        if (distance == 0L) {
            return finalRadiusBlocks;
        }
        long intervalTicks = Math.multiplyExact((long)resizeRateDays, TICKS_PER_DAY);
        long stepsElapsed = transitionTicks / intervalTicks;
        long offsetBlocks = Math.min(Math.multiplyExact(stepsElapsed, (long)resizeRateBlocks), distance);
        long signedOffset = finalRadiusBlocks >= initialRadiusBlocks ? offsetBlocks : -offsetBlocks;
        return initialRadiusBlocks + signedOffset;
    }
