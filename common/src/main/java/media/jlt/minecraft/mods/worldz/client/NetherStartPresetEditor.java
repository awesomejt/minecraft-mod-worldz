...
import media.jlt.minecraft.mods.worldz.logic.NetherStartCustomization;
...
/** Small preset editor for the {@code jlt_worldz:nether_start} typed preset (GOALS 27). */
public final class NetherStartPresetEditor implements PresetEditor {
    /** World-preset key used by loader-specific registration. */
    public static final ResourceKey<WorldPreset> NETHER_START_PRESET = ResourceKey.create(
        Registries.WORLD_PRESET,
        Identifier.parse(WorldzCommon.MOD_ID + ":nether_start")
    );
    /** Stateless editor instance shared by both loaders. */
    public static final PresetEditor INSTANCE = new NetherStartPresetEditor();
    /** Ordinary vanilla biome variety -- Nether-start is a spawn-placement mechanism, not a biome restriction. */
    ...
    private NetherStartPresetEditor() {
    }

    @Override
    public Screen createEditScreen(CreateWorldScreen parent, WorldCreationContext settings) {
        return new NetherStartCustomizeScreen(parent, currentCustomization(settings));
    }
     ...
     * @return dimensions with an ordinary Overworld and an explicit Nether-start plan
     */
    public static WorldDimensions apply(
        RegistryAccess.Frozen registries,
        WorldDimensions dimensions,
        NetherStartCustomization customization
    ) {
    ...
    private static NetherStartCustomization currentCustomization(WorldCreationContext settings) {
        ChunkGenerator netherGenerator = settings.selectedDimensions().get(LevelStem.NETHER)
            .map(LevelStem::generator)
            .orElse(null);
        if (!(netherGenerator instanceof EnvelopedChunkGenerator enveloped) || !enveloped.netherStart().enabled()) {
            return NetherStartCustomization.fromConfig(WorldzCommon.config());
        }

        NetherStartPlan netherStart = enveloped.netherStart();
