...
/** Small world-creation screen for the {@code jlt_worldz:nether_start} typed preset (GOALS 27). */
final class NetherStartCustomizeScreen extends Screen implements
    LimitEditorHosts.BorderEditorHost, LimitEditorHosts.ExteriorEditorHost, LimitEditorHosts.EndBorderEditorHost {
    private static final Component TITLE = Component.translatable("jlt_worldz.nether_start.title");
    ...
    private final String spawnYText;
    private EditBox spawnY;
    ...
    NetherStartCustomizeScreen(CreateWorldScreen parent, NetherStartCustomization initial) {
        super(TITLE);
        this.parent = parent;
        ...
        this.spawnYText = Integer.toString(initial.spawnY());
    }
    ...
    protected void init() {
        ...
        this.spawnY = textField(Component.translatable("jlt_worldz.nether_start.spawn_y"), this.spawnYText);
        form.addChild(CommonLayouts.labeledElement(
            this.font, this.spawnY, Component.translatable("jlt_worldz.nether_start.spawn_y")
        ));
    ...
    private static Component chestTierLabel(StarterKitTier tier) {
        return Component.translatable(
            "jlt_worldz.nether_start.chest_tier",
            Component.translatable("jlt_worldz.nether_start.chest_tier." + tier.serializedName())
        );
    }
    ...
    private void apply() {
        try {
            NetherStartCustomization customization = NetherStartCustomization.fromText(
                this.spawnY.getValue(),
                this.chestTier,
            ...
            this.parent.getUiState().updateDimensions(
                (registries, dimensions) -> NetherStartPresetEditor.apply(registries, dimensions, customization)
            );
            this.onClose();
