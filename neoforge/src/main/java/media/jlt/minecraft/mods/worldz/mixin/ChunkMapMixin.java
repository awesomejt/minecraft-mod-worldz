 ...
 *
 * <p>Identical to the Fabric-side mixin of the same name -- pure
 * Mojang-mapped vanilla code, no loader-specific APIs -- kept as a separate
 * per-loader mixin class rather than shared via {@code common} to match
 * this project's existing per-loader mixin convention.
 *
...
abstract class ChunkMapMixin {
    ...
    private ChunkGeneratorStructureState jltWorldz$createStateWithDelegateSettings(
        ...
        if (generator instanceof EnvelopedChunkGenerator enveloped) {
            ...
            // Same reasoning for a Nether/End chunk island (GOALS 09/37, DESIGN §29.5) -- neither
            // has a LimitedBiomeSource either; harmless no-op for the Overworld instance.
            enveloped.setChunkIslandSeed(level.getSeed());
        }
