...
| `87-nether-start-capsule-glow-lichen.yaml` | GOALS 41 (Phase 14b, DESIGN §31.9): explicit `forceCapsule: true` at an ordinary `spawnY: 32`, isolating the explicit-request pathway from config 62's automatic one; custom `capsule.sizeBlocks`/`heightBlocks` (9x9x4 interior 7x7x4) plus `glow_lichen` lighting, which coats the entire interior surface instead of spaced points. **Select "Worldz: Nether Start"**. Requires 0.3.4+. |
| `88-nether-start-capsule-hanging-lanterns.yaml` | GOALS 41 (Phase 14b, DESIGN §31.9): explicit `forceCapsule: true` at `spawnY: 32`; `sizeBlocks: 9` (7x7 interior, at/above the 6x6 "dense room" threshold) with `lantern` lighting, which always hangs from the ceiling in a real spaced grid (not just one) rather than wall-mounting, plus a second floor-standing lantern grid from the dense-room addition; hard chest tier confirms the guaranteed wooden-pickaxe essential. **Select "Worldz: Nether Start"**. Requires 0.3.4+. |
| `90-nether-start-custom-kit.yaml` | GOALS 27 (TODO 15.2a-bugfix cleanup, 2026-07-26): real in-game proof that `netherStart.hardKit` YAML overrides actually take effect — TODO 15.2a-bugfix logged this as a silently-ignored-override defect, but it was already fixed as an incidental fix in 0.3.2 and just never closed; this config is the missing real-world check. **Select "Worldz: Nether Start"**. Requires 0.3.2+. |
| `89-end-start-capsule-custom.yaml` | GOALS 34/41 (Phase 15.3, generalizing Nether-start's own capsule mechanism to `end_start`, 2026-07-25 Jason follow-up): custom `endStart.capsule.sizeBlocks`/`heightBlocks` (9x9 exterior, 7x7 interior, 4 tall) plus `lantern` lighting, isolating the platform-shape option from configs 63-65's own compiled-in defaults. Also the fix for the config-63-era bug where the player landed standing on top of the sealed platform's roof (vanilla's own same-column heightmap spawn search finding the roof before the interior) instead of inside it, missing the chest entirely -- confirm you land *inside* the room this time. **Select "Worldz: End Start"**. Requires 0.3.5+. |
| `91-end-start-capsule-torch-dense.yaml` | GOALS 34/41 (test-coverage gap, 2026-07-26): `torch` lighting — the capsule's own sanitize-time fallback — had never actually been exercised by any config for either preset; `sizeBlocks: 9` (7x7 interior) also reaches the dense-room threshold, adding a floor-standing torch grid on top of the wall ring. **Select "Worldz: End Start"**. Requires 0.3.11+. |
| `92-end-start-capsule-shroomlight-dense.yaml` | GOALS 34/41 (test-coverage gap, 2026-07-26): `shroomlight` shares its wall-embedded placement with the default `glowstone`, but no config for either preset had used an embedded source at the dense-room threshold — unlike `torch`/`lantern`, a dense embedded-source room gets *both* a floor and a ceiling grid. **Select "Worldz: End Start"**. Requires 0.3.11+. |
| `93-end-start-capsule-glow-lichen.yaml` | GOALS 34/41 (test-coverage parity, 2026-07-26): `glow_lichen` coating already had real coverage for `nether_start` (config 87) but never for `end_start` — confirms the same full-surface-coating behavior over end-stone walls. **Select "Worldz: End Start"**. Requires 0.3.11+. |
...
| `69-deep-flat-default.yaml` | GOAL 16 (Phase 16.2b): deep-flat's core mechanic — a flat surface over real, seed-driven terrain (caves, cave biomes, rivers as water). **Select "Worldz: Deep Flat"**. Requires 0.3.11+ (0.2.71-0.3.8 crashed generating almost any chunk away from spawn's own (0,0) — `applyDeepFlatCap` passed absolute world coordinates into `Heightmap.update`, which needs chunk-local 0-15; 0.3.9-0.3.10 no longer crashed but could show stray surface ponds or water draining into cave breaches, both fixed by a bounded `sealBeneathCap` pass). |
| `70-deep-flat-no-rivers.yaml` | GOAL 16 (Phase 16.2b): `riversEnabled: false` — river/ocean columns get the ordinary land cap instead of water. **Select "Worldz: Deep Flat"**. Requires 0.3.11+ (same `applyDeepFlatCap` crash/pond fixes as `69` — see its own note). |
| `71-deep-flat-structures.yaml` | GOAL 22 (Phase 16.2b): an underground structure set buried at its natural real depth by construction — contrast with `68`'s classic-flat clipped result. **Select "Worldz: Deep Flat"**. Requires 0.3.11+ (same `applyDeepFlatCap` crash/pond fixes as `69` — see its own note). |
...
| `76-stacked-void-exterior.yaml` | GOAL 35/17 (DESIGN §34.7): the bounded-world default's 64-block border paired with a VOID exterior, made explicit and actually verified (config 72 only implies it). **Select "Worldz: Stacked"**. Requires 0.3.19+. |
| `77-stacked-relief-off.yaml` | GOAL 35 (DESIGN §34.7): `reliefBlocks: 0` — restores perfectly flat per-layer surfaces, isolating relief-specific issues from layer-list issues during manual testing. **Select "Worldz: Stacked"**. Requires 0.2.77+. |
| `78-stacked-simplified-layers.yaml` | GOAL 35 (DESIGN §34.8): every layer written as a bare biome id (no `;blocks;air gap`), including one (`mushroom_fields`) with no hand-tuned `StackedBiomeDefaults` entry to exercise the generic fallback composition. **Select "Worldz: Stacked"**. Requires 0.2.77+. |
| `79-forever-night-immediate.yaml` | GOAL 30 (Phase 18.1, DESIGN §35.1): `lockAfterDays: 0` — night locks permanently at world creation; confirms sleeping can't skip it (a free consequence of the `advance_time` gamerule, not a separate mechanism) and default (vanilla) phantom rules still apply. Plain "Worldz". Requires 0.2.78+. |
| `80-forever-night-delayed-relaxed.yaml` | GOAL 30 (Phase 18.1): `lockAfterDays: 1` (delayed lock) plus `relaxInsomnia: true` (phantoms actively suppressed once locked, not just vanilla's own bed-reset). Plain "Worldz". Requires 0.2.78+. |
| `81-forever-night-border-interaction.yaml` | GOAL 30 (Phase 18.1, DESIGN §35.1's corrected known limitation): immediate night lock combined with a stepped border resize (mirrors config `24`) — confirms the resize's own periodic jumps freeze while night is locked, unlike a continuous resize (not exercised here). Plain "Worldz". Requires 0.2.78+. |
| `82-rising-lava-vanilla-limited.yaml` | GOAL 29 (Phase 18.2, DESIGN §35.2): rising lava on a plain bordered "vanilla-limited" world (mirrors config `13`), accelerated rate for testability — confirms the delay/rise/max schedule, air/water-only conversion, and that a chunk visited later still catches up to the current level. Plain "Worldz". Requires 0.2.79+. |
| `83-rising-lava-ocean-island.yaml` | GOAL 29 (Phase 18.2): rising lava composed with a typed preset instead of the plain preset — confirms the hazard applies uniformly, flooding the real ocean/underground from the bottom up. **Select "Worldz: Ocean Island"**. Requires 0.2.79+. |
| `84-structure-distance-basic.yaml` | GOAL 24 (Phase 19.2, DESIGN §36.2-§36.3): every vanilla structure set held back from spawn by a shared minimum distance (shrunk to 400 blocks for testability). Confirms `/locate structure` may still report a coordinate inside the radius (it predicts from placement math alone, unaware of the suppression) while nothing actually generates there, and that structures return to normal beyond the radius. Plain "Worldz". Requires 0.2.82+. |
| `85-structure-distance-single-biome-exempt-stronghold.yaml` | GOAL 24 (Phase 19.2): composes the same module with a typed preset (proving it applies to any world type with no per-preset wiring) and exercises `exemptStructureSets` — strongholds still generate at their normal vanilla distance even inside the restricted radius that suppresses everything else. **Select "Worldz: Single Biome"**. Requires 0.2.82+. |
| `86-cave-sealed-surface-bedrock.yaml` | GOAL 25 (Phase 13.2b follow-up, Jason 2026-07-25): configurable sealed-surface block/thickness — the same roof as config `54` but built from bedrock at 3 blocks thick instead of the default 5-thick stone, so it genuinely cannot be mined through. **Select "Worldz: Cave"**. Requires 0.2.88+. |
| `94-flat-slime-cavity.yaml` | GOAL 15 (Phase 16.9, GOALS 15 clarification, 2026-07-26): a raised, slime-free surface (Y 60) with a big hollow, enclosed slime-spawning cavity carved out between Y -14 and Y 35 — proves a `flat.layers` entry can already be `minecraft:air`, not just the top one, so no dedicated "void" layer type is needed. Also lists every vanilla Overworld structure set in `structureOverrides`, which (biome `plains`) triggers the new Phase 16.10 "will never generate" warning for `ancient_cities`. **Select "Worldz: Flat"**. Requires 0.3.11+ (documentation/example only, no code change). |
| `95-flat-structure-override-biome-match.yaml` | GOALS 15/22 (Phase 16.10, 2026-07-26): the flip side of configs 68/94 — `flat.biome: minecraft:deep_dark` actually matches the one `structureOverrides` entry (`minecraft:ancient_cities`), so the new automatic biome-mismatch log warning stays silent and the structure really generates. **Select "Worldz: Flat"**. Requires 0.3.12+ (first version with the automatic warning). |
